...
@section('content')
    ...
    @endif
    <form class="form-section formboxbg" id="form-league" method="POST" action="{{ route('admin.teams.update', [$team->id, $language_id, $default_language_post_id]) }}" enctype="multipart/form-data">
        @csrf
        ...
        <div class="mainform-sec">
            <div class="input-group">
                ...
                <input type="text" class="form-control name" id="post_title" name="post_title"
                    value="{{ old('post_title', $team->post_title) }}" placeholder="Team Title">
                @error('post_title')
            ...
            <div class="input-group">
                ...
                <input type="text" class="form-control slug" id="team_id" name="team_id"
                    value="{{ old('team_id', $team->team_id) }}" placeholder="Team id">
                @error('team_id')
                    <div class="text-danger">{{ $message }}</div>
                @enderror
            </div>
            <div class="input-group">
                ...
                <input type="text" class="form-control slug" id="divanscore_home_id" name="divanscore_home_id"
                    value="{{ old('divanscore_home_id', $team->divanscore_home_id) }}" placeholder="Divanscore Home ID">
            ...
            <div class="input-group">
                ...
                <input type="text" class="form-control slug" id="divanscore_away_id" name="divanscore_away_id"
                    value="{{ old('divanscore_away_id', $team->divanscore_away_id) }}" placeholder="Divanscore Away ID">
            ...
            <div class="input-group">
                ...
                <input type="text" class="form-control slug" id="divanscore_tournament_id" name="divanscore_tournament_id"
                    value="{{ old('divanscore_tournament_id', $team->divanscore_tournament_id) }}" placeholder="Divanscore Tournament ID">
            ...
            <div class="input-group">
                ...
                <input type="text" class="form-control slug" id="divanscore_season_id" name="divanscore_season_id"
                    value="{{ old('divanscore_season_id', $team->divanscore_season_id) }}" placeholder="Divanscore Season ID">
            ...
            <div class="input-group">
                ...
                @if ($team->team_logo)
                    <div class="mb-3">
                        <img src="{{ asset('assets/images/team_logo/' . $team->team_logo) }}" width="100">
                    </div>
                @endif
            </div>
            <div class="input-group">
                ...
                <input type="text" class="form-control slug" id="team_country" name="team_country"
                    value="{{ old('team_country', $team->team_country) }}" placeholder="Team Country">
                @error('team_country')
                    <div class="text-danger">{{ $message }}</div>
                @enderror
            </div>
            <div class="input-group">
                ...
                <input type="text" class="form-control slug" id="team_city" name="team_city"
                    value="{{ old('team_city', $team->team_city) }}" placeholder="Team City">
                @error('team_city')
                    <div class="text-danger">{{ $message }}</div>
                @enderror
            </div>
            <div class="input-group">
                ...
                <input type="text" class="form-control slug" id="team_venue" name="team_venue"
                    value="{{ old('team_venue', $team->team_venue) }}" placeholder="Team Venue">
                @error('team_venue')
                    <div class="text-danger">{{ $message }}</div>
                @enderror
            </div>
            <div class="input-group">
                ...
                <input type="text" class="form-control slug" id="team_manager" name="team_manager"
                    value="{{ old('team_manager', $team->team_manager) }}" placeholder="Team Manager">
                @error('team_manager')
                    <div class="text-danger">{{ $message }}</div>
                @enderror
            </div>
                ...
                <select class="form-select country_id" id="season_id" name="season_id">
                    ...
                    @foreach ($seasons as $season)
                        <option value="{{ $season['id'] }}"
                            {{ old('season_id', $team->season_id) == $season['id'] ? 'selected' : '' }}>
                            {{ $season['name'] }}
                        </option>
                    @endforeach
                </select>
                ...
                <select class="form-select language_id" id="language_id" name="language_id">
                    ...
                    @foreach ($languages as $language)
                        <option value="{{ $language['id'] }}"
                            {{ old('language_id', $team->language_id) == $language['id'] ? 'selected' : '' }}>
                            {{ $language['fullname'] }}
                        </option>
                    @endforeach
                </select>
                        ...
                        @foreach($team->players as $index => $player)
                            ...
                            <div class="player-item form-section formboxbg mb-3">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="hidden" name="players[{{ $index }}][id]" value="{{ $player->id }}">

                                    <input type="hidden" name="players[{{ $index }}][existing_photo]" value="{{ $player->photo }}">


                                    @if($player->photo)
                                        <div class="mt-2">
                                            <img src="{{ asset('assets/images/player_photo/'.$player->photo) }}"
                                                width="80">
                                        </div>
                                    @endif
...
<script>
    $(document).ready(function () {
        let playerIndex = {{ count($team->players ?? []) }};
        $('#add-player').click(function () {

            let html = `
            <div class="player-item form-section formboxbg mb-3">

                <input type="hidden" name="players[${playerIndex}][id]" value="">
                ...
                <div class="input-group">
                    ...
                    <input type="text"
                        class="form-control"
                        name="players[${playerIndex}][position]">
                </div>
                ...
                <div class="input-group">
                    ...
                    <input type="text"
                        class="form-control"
                        name="players[${playerIndex}][rating]">
                </div>
                ...
                <div class="input-group">
                    ...
                    <input type="file"
                        class="form-control"
                        name="players[${playerIndex}][photo]">
                </div>
