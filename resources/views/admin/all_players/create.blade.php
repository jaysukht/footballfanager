    ...
    @section('content')
        ...
        @endif
        <form class="form-section formboxbg" id="form-league" method="POST" action="{{ route('admin.all-players.store') }}" enctype="multipart/form-data">
            @csrf
                ...
                <div class="input-group w-100">
                    ...
                    <div id="player-wrapper" class="w-100">
                        ...
                        <div class="player-item form-section formboxbg d-flex align-items-center gap-2 mb-2 flex-wrap">
                            ...
                            <button type="button"
                                class="btn btn-danger remove-player">
                                Remove
                            </button>
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    let addBtn = document.getElementById("add-player");
    let index = 1;

    addBtn.addEventListener("click", function () {

        let newRow = document.createElement("div");
        newRow.classList.add("player-item", "form-section", "formboxbg", "d-flex", "align-items-center", "gap-2", "mb-2", "flex-wrap");

        newRow.innerHTML = `
            <input type="number" class="form-control" name="players[${index}][player_id]" placeholder="Player ID">
            <input type="text" class="form-control" name="players[${index}][first_name]" placeholder="First Name">
            <input type="text" class="form-control" name="players[${index}][last_name]" placeholder="Last Name">
            <input type="text" class="form-control" name="players[${index}][team_name]" placeholder="Team Name">
            <input type="number" class="form-control" name="players[${index}][matches]" placeholder="Matches">
            <input type="number" class="form-control" name="players[${index}][goals]" placeholder="Goals">
            <input type="number" class="form-control" name="players[${index}][assists]" placeholder="Assists">
            <input type="text" class="form-control" name="players[${index}][player_image]" placeholder="Player Image URL">
            <input type="text" class="form-control" name="players[${index}][team_image]" placeholder="Team Image URL">
            <input type="text" class="form-control" name="players[${index}][position]" placeholder="Position">
            <input type="text" class="form-control" name="players[${index}][country]" placeholder="Country">
    ...
    playerWrapper.addEventListener("click", function (e) {
        if (e.target.classList.contains("remove-player")) {
            ...
            let allRows = document.querySelectorAll(".player-item");
